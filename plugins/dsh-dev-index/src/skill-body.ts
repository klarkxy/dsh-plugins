...
export function renderSkillBody(pagesBaseUrl: string): string {
  const lines = [
    ...
    "Read the index before editing code. Do not invent APIs. This skill does not bundle a copy of the docs.",
    "",
    "Fetch the always-current English index from GitHub Pages:",
    `- ${pagesBaseUrl}llms.txt`,
    `- ${pagesBaseUrl}index.json`,
    `- ${pagesBaseUrl}meta.json`,
    `- ${pagesBaseUrl}areas/<id>.md`,
    "",
    "index.json is the catalog: area id, summary, and official source paths. meta.json records officialTag and officialCommit for the DeepSeek Harness revision those pages describe. Read index.json, then open the one area file the task needs. English is the default.",
    "Simplified Chinese human-readable pages exist under zh/ on the same hosts (zh/llms.txt and zh/areas/<id>.md). index.json and meta.json stay in English.",
    "",
