...
export function renderSkillBody(pagesBaseUrl: string): string {
  const lines = [
    ...
    "2. Read meta.json. officialTag and officialCommit are the DSH revision these pages were written against. If the target version differs, treat every page as unverified for that version and check type declarations and source pinned to the target commit. Do not silently substitute this index for another version.",
    ...
    "4. Outside DSH, do not assume those tools exist. Read the docs, type declarations, and source pinned to the target version. Mark every conclusion that could not be runtime-verified.",
    ...
    "",
    "index.json lists task ids, guide ids, and area ids. meta.json records officialTag and officialCommit. English is the default. Simplified Chinese human-readable pages live under zh/ (zh/llms.txt, zh/tasks/<id>.md, zh/areas/<id>.md). index.json and meta.json stay in English.",
    "",
